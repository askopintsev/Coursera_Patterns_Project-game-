...
class MapFactory(yaml.YAMLObject):
    ...
    def from_yaml(cls, loader, node):
        _map = cls.Map()
        _obj = cls.Objects()
        enemy_on_lvl = loader.construct_mapping(node, deep=True)
        _obj.config.update(enemy_on_lvl)
            ...
            for obj_name in object_list_prob['enemies']:
                ...
                for i in range(random.randint(0, 5)):
                    ...
                    self.objects.append(Objects.Enemy(
                        prop['sprite'], prop, prop['experience'], coord, obj_name))
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            ...
            for obj_name in self.config.keys():
                prop = object_list_prob['enemies'][obj_name]
                for i in range(self.config[obj_name]):  # get count
                    coord = (random.randint(1, 30), random.randint(1, 22))
                    intersect = True
                    while intersect:
                        intersect = False
